...
from gnn.imdbmulti_model import PershomLearnedFilt, PershomLearnedFiltSup, PershomRigidDegreeFilt, GIN, SimpleNNBaseline, ClassicGNN, ClassicReadoutFilt
...
if __name__ == "__main__":
    print("IMDB-MULTI TRAIN")
    parser = argparse.ArgumentParser(description='IMDB-MULTI')
    parser.add_argument('--device', type= int, default= 0)

    parser.add_argument('--max_process_on_device', type=int)

    parser.add_argument('--exp_name', type= str, default= 'imdb-multi', help= 'experiment name to save as')
    parser.add_argument('--dataset_name', type= str, default= 'IMDB-MULTI')
    ...
    parser.add_argument('--use_raw_node_label', type= arg_bool, default= False)
    parser.add_argument('--use_node_degree', type=arg_bool, default=True)
    parser.add_argument('--set_node_degree_uninformative', type=arg_bool, default=False)
    parser.add_argument('--use_node_label', type=arg_bool, default=False)
    ...
    if args.readout=="extph" or args.readout== 'extph_cyclereps':
        model = PershomLearnedFiltSup(dataset, args.use_super_level_set_filtration, args.use_node_degree,
                                       args.set_node_degree_uninformative, args.use_node_label, args.use_raw_node_label,
                                       ...
                                       conv_number=args.conv_number, conv_dimension=args.conv_dimension, aug=None, readout= args.readout, use_bars= args.bars).to(device)
    else:
