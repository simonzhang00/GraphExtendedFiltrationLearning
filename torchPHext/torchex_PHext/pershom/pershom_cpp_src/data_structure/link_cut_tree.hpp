...
public:
    ...
    /**
     * DOENST WORK!!! left and right can both be -1 even if they aren't...
     * @description make `a` not the root of the represented tree by making one of its children the root
     */
    void avoid_root(int a) {
        std::cout<<"avoid_root inside: "<<left[a]<<" "<<right[a]<<std::endl;
        if(left[a]!=-1){
            evert(left[a]);
        }else if(right[a]!=-1){
            evert(right[a]);
