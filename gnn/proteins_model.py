...
class ClassicGNN(torch.nn.Module):
    def __init__(self,
        ...
        self.embed_lab = nn.Embedding(num_node_lab, dim) if use_node_label and num_node_lab is not None else None
        ...
        if use_raw_node_label and dataset[0].x is not None:
            self.node_label_embedder = torch.nn.Linear(dataset[0].x.size(1), gin_dimension)
...
class PershomBase(nn.Module):
    ...
    def forward(self, batch, device):
        ...
        if self.readout_type == "extph":
            ...
            node_filt0 = node_filt0.unsqueeze(1)
            g1 = global_sort_pool(node_filt0, batch.batch, 400)
        out = g1
...
class ClassicReadoutFilt(PershomBase):
    def __init__(self,
        ...
        if self.readout_type == "sort":
            self.classifier_gnn = ClassifierHead(dataset, 400, cls_hidden_dimension, drop_out=drop_out)
        else:
...
class GIN(nn.Module):
    def __init__(self,
        ...
        self.embed_lab = OneHotEmbedding(num_node_lab) if use_node_label and num_node_lab is not None else None
