...
class ClassicGNN(torch.nn.Module):
    def __init__(self,
        ...
        #dim_input = dim * ((self.embed_deg is not None))
        if (use_node_label and use_raw_node_label):
...
class MolFiltration(torch.nn.Module):
    def __init__(self,
                 dataset,
                 use_node_degree=None,
                 set_node_degree_uninformative=None,
        ...
        self.gnn_node = GNN_node_Virtualnode(filt_conv_number, filt_conv_dimension)

    def reset_parameters(self):
        # for conv in self.gnn_node.convs:
        #     conv.reset_parameters()
        for mlp in self.gnn_node.mlp_virtualnode_list:
    ...
    def forward(self, batch):
        assert(hasattr(batch, "x") and batch.x is not None)
        #if hasattr(batch, "x") and batch.x is not None:
        h = self.gnn_node((batch.x, batch.edge_index, batch.edge_attr, batch.batch, self.embed_deg(batch.node_deg)))
        # else:
        #     h = self.gnn_node((self.embed_deg, batch.edge_index, batch.edge_attr, batch.batch))
        return h
    ...
    def forward(self, batch):
        ...
        if self.use_raw_node_label:
            if self.embed_deg is not None and self.fc is not None:
                # tmp = [self.embed_deg(node_deg),self.node_label_embedder(batch.x.to(torch.float))]
                tmp = [self.embed_deg(node_deg), batch.x]  # self.node_label_embedder(batch.x.to(torch.float))]
            else:
                tmp = [batch.x]
        else:
            tmp = [e(x) for e, x in
...
class PershomReadout(nn.Module):
    def __init__(self,
        ...
        self.ldgm_cc = SLayerRationalHat(num_struct_elements, 2,
                                         radius_init=0.1)  # SLayerExponential(num_struct_elements, 2)#SLayerSquare(num_struct_elements, 2)
