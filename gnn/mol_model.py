...
from .conv import GNN_node_Virtualnode, AtomEncoder
...
class Filtration(torch.nn.Module):
    def __init__(self,
        ...
        self.atom_encoder = AtomEncoder(emb_dim= dataset[0].x.size(1))
        if dim_input==0:
            raise Exception("Cannot have neither degree nor node features")
        elif 0 in dims:
            raise Exception("Cannot have 0 dim hidden layers")
...
class PershomLearnedFiltSup(PershomBase):
    def __init__(self,
        ...
        self.lstm = torch.nn.LSTM(1, 2 * num_struct_elements, num_layers=2, bidirectional=True)
